    ...
    if data_type == 'indicators':
        for category, items in findings.items():
            ...
            for value, context in items.items():
                ...
                if 'File:' in context: file_source = context.split('File:')[1].split('|')[0].strip()
                if 'Position:' in context: position = context.split('Position:')[1].split('|')[0].strip()
                data.append({
                    'category': category, 'value': value, 'details': context, 'file': file_source, 'position': position
                })
