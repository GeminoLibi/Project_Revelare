...
def process_project(project_name: str, input_files: List[str], output_dir: str) -> bool:
    ...
    try:
        ...
        # String search if requested
        if args.search_strings:
            print(f"\n[SEARCH] Performing string search for {len(args.search_strings)} strings...")
            try:
                from string_search import StringSearchEngine
                
                search_engine = StringSearchEngine(cli_logger)
                search_results = search_engine.search_directory(
                    str(extracted_files_dir), 
                    args.search_strings, 
                    args.search_context,
                    args.search_extensions
                )
                
                if search_results:
                    search_output = os.path.join(project_dir, f"{safe_project_name}_string_search.csv")
                    search_engine.save_results_to_csv(search_results, search_output)
                    print(f"[OK] [OK] String search results: {os.path.basename(search_output)} ({len(search_results)} matches)")
                else:
                    print(f"[OK] [OK] String search completed: No matches found")
                    
            except Exception as e:
                cli_logger.error(f"String search failed: {e}")
                print(f"[WARNING] String search failed: {e}")
        
        # MBOX analysis if requested
        if args.mbox_analyze:
            print(f"\n[MBOX] Analyzing MBOX file: {args.mbox_analyze}")
            try:
                from mbox_viewer import MboxViewer
                
                viewer = MboxViewer()
                analysis = viewer.analyze_mbox_file(args.mbox_analyze)
                
                if analysis:
                    # Export analysis
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    json_file = os.path.join(project_dir, f"{safe_project_name}_mbox_analysis_{timestamp}.json")
                    html_file = os.path.join(project_dir, f"{safe_project_name}_mbox_analysis_{timestamp}.html")
                    csv_file = os.path.join(project_dir, f"{safe_project_name}_mbox_analysis_{timestamp}.csv")
                    
                    viewer.export_to_json(analysis, json_file)
                    viewer.generate_html_report(analysis, html_file)
                    viewer.export_to_csv(analysis, csv_file)
                    
                    print(f"[OK] MBOX analysis complete: {analysis['total_messages']} messages processed")
                    print(f"[OK] Results saved: {os.path.basename(json_file)}, {os.path.basename(html_file)}, {os.path.basename(csv_file)}")
                else:
                    print(f"[ERROR] MBOX analysis failed")
                    
            except Exception as e:
                cli_logger.error(f"MBOX analysis failed: {e}")
                print(f"[WARNING] MBOX analysis failed: {e}")
        
        # MBOX search if requested
        if args.mbox_search and args.mbox_analyze:
            print(f"\n[MBOX] Searching MBOX file for {len(args.mbox_search)} terms...")
            try:
                from mbox_viewer import MboxViewer
                
                viewer = MboxViewer()
                search_results = viewer.search_messages(args.mbox_analyze, args.mbox_search, args.mbox_fields)
                
                if search_results:
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    csv_file = os.path.join(project_dir, f"{safe_project_name}_mbox_search_{timestamp}.csv")
                    ...
                    print(f"[OK] MBOX search complete: {len(search_results)} matches found")
                    print(f"[OK] Results saved: {os.path.basename(csv_file)}")
                else:
                    print(f"[OK] MBOX search completed: No matches found")
                    
            except Exception as e:
                cli_logger.error(f"MBOX search failed: {e}")
                print(f"[WARNING] MBOX search failed: {e}")
...
Examples:
  ...
  python revelare_cli.py --add-files "case_001" --files evidence.zip new_docs.pdf
        """
    )
    ...
    parser.add_argument('--add-files', help='Add files to existing case (specify case name)')

    # String search options
    parser.add_argument('--search-strings', nargs='+', help='Search for specific strings in files')
    parser.add_argument('--search-context', type=int, default=50, help='Context characters around string matches (default: 50)')
    parser.add_argument('--search-extensions', nargs='+', help='File extensions to search (e.g., .txt .log .csv)')
    
    # MBOX viewer options
    parser.add_argument('--mbox-analyze', help='Analyze MBOX file (provide path to .mbox file)')
    parser.add_argument('--mbox-search', nargs='+', help='Search terms for MBOX file analysis')
    parser.add_argument('--mbox-fields', nargs='+', choices=['subject', 'from', 'to', 'body_text', 'body_html'], 
                       default=['subject', 'from', 'to', 'body_text'], help='Fields to search in MBOX messages')
    ...
    if args.onboard:
        # Interactive case creation with onboarding
        print("\n" + "="*60)
        print("Project Revelare - Case Onboarding & Creation")
        print("="*60)
        ...
        if success:
            ...
            if evidence_files:
                # Process evidence files immediately
                ...
                success, process_msg = case_manager.process_evidence_files(
                    project_name, evidence_files, progress_callback
                )

                if success:
                    print(f"[OK] {process_msg}")
                    case_manager.onboard.display_next_steps(project_dir, project_name)
                else:
                    print(f"[ERROR] {process_msg}")
            else:
                print("[INFO] No evidence files added. Run processing later with --process command.")
        else:
    ...
    if args.add_files:
        ...
        print(f"\n{'='*60}")
        print(f"Project Revelare - Add Files to Case: {case_name}")
        print(f"{'='*60}")

        # Validate case exists
        available_cases = case_manager.get_available_cases()
        case_names = [case['name'] for case in available_cases]

        if case_name not in case_names:
            print(f"[ERROR] Case '{case_name}' not found.")
            print(f"Available cases: {', '.join(case_names)}")
            return 1

        # Validate and process files
        valid_files, total_size = validate_input_files(files)

        if not valid_files:
            print("[ERROR] No valid files found")
            return 1

        print(f"Adding {len(valid_files)} files to case '{case_name}'")
        ...
        print("\nProcessing files...")
        success, message = case_manager.process_evidence_files(case_name, valid_files, progress_callback)

        if success:
            print(f"[OK] {message}")
            print(f"\nTip: Use 'python start.py' to view the updated case in the web interface")
        else:
            print(f"[ERROR] {message}")
            return 1
